...
module Group_table :
  S
    with type key = [ `Id of Types.Group.id | `Name of Types.Group.name ]
     and type value = Types.Group.id

module Environment_table : sig
  include
    S
      with type key =
        [ `Id of Types.Environment.id | `Name of Types.Environment.name ]
       and type value = Types.Environment.id
...
module Feature_table : sig
  include
    S
      with type key = [ `Id of Types.Feature.id | `Name of Types.Feature.name ]
       and type value = Types.Feature.id

  val set_feature : feature:Types.Feature.t -> t -> t
end

module Composite_environment_table :
  S with type key = Types.Group.id and type value = Environment_table.t

module Composite_feature_table :
  S with type key = Types.Group.id and type value = Feature_table.t
