      ...
      ( "is_schedule_active",
        [
          ...
          (* test_case "schedule start end - before" `Quick (fun () ->
                 ...
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let two_days_from_now =
                   Ptime.add_span today (Ptime.Span.add one_day_s one_day_s)
                   |> Option.get  |> Date_time.to_ms_exn
                 in
                 let schedule =
                   Types.Schedule.make ~start:tomorrow ~end':two_days_from_now
                     ~timezone:Utc ~time_type:Start_end ~start_time:0
                     ~end_time:0
                 in
             ...
             test_case "schedule start end - during" `Quick (fun () ->
                 let today = Ptime_clock.now () in
                 let yesterday = Ptime.sub_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let schedule =
             ...
             test_case "schedule start end - after" `Quick (fun () ->
                 let today = Ptime_clock.now () in
                 let yesterday = Ptime.sub_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let two_days_ago = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let schedule =
             ...
             test_case "schedule active daily - before whole schedule" `Quick
               (fun () ->
                 ...
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let two_days_from_now =
                   Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn
                 in
                 let schedule =
                   Types.Schedule.make ~start:tomorrow ~end':two_days_from_now
                     ~timezone:Utc ~time_type:Daily ~start_time:0 ~end_time:0
                 in
                 let expected = false in
                 let actual =
                   Schedule.is_schedule_active ~schedule ~schedule_type:Global
             ...
             test_case "schedule active daily - before day schedule" `Quick
               (fun () ->
                 ...
                 let yesterday = Ptime.sub_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let start_time = Ptime.add_span Date_time.epoch one_hour_s |> Option.get |> Date_time.to_ms_exn in
                 let end_time =
                   Ptime.add_span Date_time.epoch (Ptime.Span.add one_hour_s one_hour_s)
                   |> Option.get |> Date_time.to_ms_exn
                 in
                 let schedule =
                   Types.Schedule.make ~start:yesterday ~end':tomorrow
             ...
             test_case "schedule active daily - during day schedule" `Quick
               (fun () ->
                 ...
                 let yesterday = Ptime.sub_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let start_time = Ptime.sub_span Date_time.epoch one_hour_s |> Option.get |> Date_time.to_ms_exn in
                 let end_time = Ptime.add_span Date_time.epoch one_hour_s |> Option.get |> Date_time.to_ms_exn in
                 let schedule =
                   Types.Schedule.make ~start:yesterday ~end':tomorrow
                     ~timezone:Utc ~time_type:Daily ~start_time ~end_time
                 in
             ...
             test_case "schedule active daily - after day schedule" `Quick
               (fun () ->
                 ...
                 let tomorrow = Ptime.add_span today one_day_s |> Option.get |> Date_time.to_ms_exn in
                 let start_time =
                   Ptime.sub_span Date_time.epoch (Ptime.Span.add one_hour_s one_hour_s)
                   |> Option.get |> Date_time.to_ms_exn
                 in
                 let end_time = Ptime.sub_span Date_time.epoch one_hour_s |> Option.get |> Date_time.to_ms_exn in
                 let schedule =
                   Types.Schedule.make ~start:yesterday ~end':tomorrow
                     ~timezone:Utc ~time_type:Daily ~start_time ~end_time
                 in
